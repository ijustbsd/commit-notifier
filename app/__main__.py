...
async def handle_commits(gh_client, tg_bot_client):
    ...
    for person in persons:
        commits = await gh_client.get_commits(person.github_username, person.github_repo)
        commits = commits[::-1]
        ...
        commits_text = '\n'.join([
            f"{n}. [{c['message']}]({c['url']})" for n, c in enumerate(new_commits, 1)
        ])

        message = f'*{person.full_name}*:\n' + commits_text

        await tg_bot_client.send_message(message, settings.TG_CHAT_ID, {
            'parse_mode': 'Markdown',
